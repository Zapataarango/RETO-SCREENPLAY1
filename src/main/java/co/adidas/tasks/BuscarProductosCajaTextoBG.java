...
public class BuscarProductosCajaTextoBG implements Task {
    BuscarProductosCajaTextoOT_UI buscarProductosCajaTextoOTUI = new BuscarProductosCajaTextoOT_UI();

    private final String Producto;


    public BuscarProductosCajaTextoBG(String producto) {
        Producto = producto;
    }

    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(Enter.theValue(Producto).into(buscarProductosCajaTextoOTUI.TXT_BARRA_BUSQUEDA),
    ...
    public static Performable enAdidas(String Producto) {
        return Instrumented.instanceOf(BuscarProductosCajaTextoBG.class).withProperties(Producto);
    }
